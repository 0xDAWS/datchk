...
class rom_checker:
    ...
    def compare(self, rom, checksum) -> None:
        if checksum is not None:
            self.valid = compare_checksum(rom, checksum, args.algorithm, self.tmpdir)
        else:
            self.csna = True
...
def main():
    ...
    if args.check:
        ...
        if check_results["PROC"] > 0:
            console.print(
                "{:<15} {}".format("Processed:", check_results["PROC"]),
                style="bold blue",
            )
        if check_results["FAIL"] > 0:
            console.print(
                "{:<15} {}".format("Failed:", check_results["FAIL"]), style="bold red"
            )
        if check_results["CSNA"] > 0:
            console.print(
                "{:<15} {}".format("Checksum N/A:", check_results["CSNA"]),
                style="bold yellow",
            )
        if check_results["NIDF"] > 0:
            console.print(
                "{:<15} {}".format("Not in datfile:", check_results["NIDF"]),
                style="bold magenta",
            )
